<template>
<section class="wb-command-hero"><div><span class="wb-eyebrow">A-SHARE / DAILY WORKFLOW</span><h2>先确认数据，再做今天的计划。</h2><p>盘后收拢方向，竞价核对强弱，10点前观察入场。每一步都能看到数据是否准备好。</p><div class="wb-inline"><button class="wb-button primary" @click="$emit('open','research')">进入次日备选 →</button><button class="wb-button" @click="$emit('open','screen','pre')">查看竞价确认</button></div></div><div class="wb-readiness-score"><span>关键数据检查</span><strong>{{ loading?'…':report?.ready ?? '—' }}<small>/ {{ report?.total || 7 }}</small></strong><span>是数据项检查，不是策略胜率</span></div></section>
...
<section class="wb-bottom-grid"><article class="wb-panel"><h3>使用前仍需补足</h3><p v-for="s in report.limits" :key="s" class="wb-checkline"><span>○</span>{{ s }}</p></article><article class="wb-panel"><h3>采集运行方式</h3><p class="wb-muted">{{ report.automation }}</p><p class="wb-muted">页面刷新只读本地；补采按钮才调用量脉。单次股票数有限制，任务互斥，错误与旧日期都会明确显示。</p><button class="wb-button small" @click="$emit('open','data')">查看接口采集证据 ↗</button></article></section>
</template>
</template>
<script setup>
import {ref,watch} from 'vue'
import axios from 'axios'
const props=defineProps({day:String,busy:Boolean});const emit=defineEmits(['open','collect'])
