<template>
 <div class="wb-intraday">
  <div class="wb-panel-title"><div><h3 v-if="!compact">当日走势 <small>5分钟采样</small></h3><span>价格线 · 昨收线 · 成交量</span></div><div class="wb-mode"><button :class="{active:early}" @click="early=true">10点前</button><button :class="{active:!early}" @click="early=false">全天</button></div></div>
  ...
  <template v-else-if="points.length">
   ...
   <div class="wb-intraday-canvas"><svg viewBox="0 0 720 355" role="img" aria-label="当日五分钟价格走势与成交量">
    ...
    <line :x1="x(10)" :x2="x(10)" y1="22" y2="254" stroke="#a7977355" stroke-dasharray="3 4"/><text :x="x(10)-5" text-anchor="end" y="18" fill="#d3b575" font-size="11">09:40</text>
    <line :x1="x(30)" :x2="x(30)" y1="22" y2="254" stroke="#a7977355" stroke-dasharray="3 4"/><text :x="x(30)+(early?-4:5)" :text-anchor="early?'end':'start'" y="18" fill="#d3b575" font-size="11">10:00</text>
  ...
  <details class="wb-chart-note"><summary>5分钟采样 · 数据口径</summary><p class="wb-footnote">{{ chart?.note }} {{ chart?.scope==='early'?'当前仅有早盘缓存，可补采全天。':'' }}</p></details>
 </div>
</template>
<script setup>
import {ref,computed,watch} from 'vue'
const props=defineProps({chart:Object,loading:Boolean,compact:Boolean});const early=ref(!props.compact),hover=ref(null)
