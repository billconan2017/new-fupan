<template>
<section class="wb-panel"><div class="wb-panel-title"><div><h3>最近五个交易日 · 早盘回溯</h3><span>{{ report?.dates?.join(' / ') }} · 后续行情截至 {{ report?.asof || '—' }}</span></div><button class="wb-button" @click="load">刷新回溯结果</button></div>
<p v-if="error" role="alert" class="wb-notice error">{{ error }}</p><p v-if="!report?.complete" class="wb-notice">回溯尚未完成，以下可能为部分结果。</p>
<p>{{ report?.note }}</p><div class="wb-rules"><strong>先固定规则，再看结果</strong><p>前日涨停池 / 强势池 / 龙虎榜有交集 → 当日竞价涨幅1%—8%、规则分≥60 → 排名前3只普通沪深主板股票 → 09:35首根五分钟收阳 → 模拟09:40入场。未触发不补选，涨跌停或数据缺失单列。</p></div>
...
<div class="wb-panel-title"><h3>逐日候选与模拟结果</h3><select v-model="selectedDay" aria-label="回溯日期筛选"><option value="">全部日期</option><option v-for="d in report?.dates || []" :key="d">{{ d }}</option></select></div>
<div v-for="d in visibleDays" :key="d.date" class="wb-rules"><strong>{{ d.date }} · 前日证据 {{ d.previous_date }} · 竞价 {{ d.auction_rows }}条 · 入选 {{ d.picked }}只</strong><small v-for="s in d.sources" :key="s.api">{{ s.api }}：{{ s.count }}条 / {{ sourceNames[s.status] || s.status }}　</small><p v-if="!d.picked">没有合格候选或来源核验未通过，保留空仓。</p></div>
...
async function load(){error.value='';try{report.value=(await axios.get('/api/workbench/recent-replay')).data}catch{error.value='回溯结果读取失败，请重试。'}}
onMounted(load)
</script>
