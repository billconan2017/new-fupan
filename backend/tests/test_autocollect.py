...
def test_no_lunch_weekend_or_late_auction():
    ...
    assert slots(at('2026-09-17T09:26:00'))==[('09:26','pre'),('09:26','paper')]
...
def test_coalesce_intervals_and_latest_close_slot():
    ...
    assert slots(at('2026-09-17T19:00:00'))==[('17:30',s) for s in ('review','paper','intraday','history')]
    assert slots(at('2026-09-17T23:59:00'))==[('22:00',s) for s in ('review','paper','intraday','history')]
...
async def test_calendar_gate_and_durable_duplicate_prevention(monkeypatch):
    ...
    monkeypatch.setattr(ac,'slots',lambda now:[('09:26','pre')])
    try:
